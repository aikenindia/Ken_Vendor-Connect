...
EXAMPLE (showing the "capture everything" principle):
Input across conversation: "Count: 30s Combed, Mill Name: (not given), Qty: 1000 kg, Delivery: Ex-Mill, GST: Extra, Validity: 7 Days, Payment: Advance"
-> message_body:
"Count      : 30s Combed
Qty        : 1000 kg
Delivery   : Ex-Mill
...
Validity   : 7 Days
Payment    : Advance"
(Note: "Mill Name" was omitted entirely since no value was actually given for it — not written as blank or null.)
...
STRICT RULES:
1. If the conversation clearly describes only ONE distinct item, set type="enquiry". ready=true the moment that item has a real item name (or Count for yarn) anywhere in the combined conversation. items array has exactly 1 entry. vendor_ids always [].
2. If the conversation contains TWO OR MORE distinct items/materials (including multiple yarn Count entries), set type="multiple", ready=false, populate the full items array, and set clarification to a short question confirming whether to split into separate enquiries.
